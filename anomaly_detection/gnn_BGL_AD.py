...
def build_graph_data_for_trace(events_df, edges_df, traces_df, task_id, eid_to_ix=None):
    ...
    ts = ev["Timestamp"].astype("int64")
    # Normalize timestamps within the trace (0..1)
    tsn = (ts - ts.min()) / max(1, (ts.max() - ts.min()))

    # Optionally: normalized position in the trace
    pos = np.arange(len(ev), dtype="float32")
    posn = pos / max(1, (len(ev) - 1))

    # In/out degree (based on edges)
    indeg = pd.Series(0, index=tids); outdeg = pd.Series(0, index=tids)
    for _, r in ed.iterrows():
        if str(r["FatherTID"]) in idx:
            outdeg[str(r["FatherTID"])] += 1
        if str(r["ChildTID"]) in idx:
            indeg[str(r["ChildTID"])] += 1

    indeg_d, outdeg_d = indeg.to_dict(), outdeg.to_dict()

    x_num = np.c_[
        tsn.to_numpy(),                                      # normalized timestamp
        posn,                                                # normalized position in trace
        ev["TID"].astype(str).map(indeg_d).fillna(0).to_numpy(),
        ev["TID"].astype(str).map(outdeg_d).fillna(0).to_numpy(),
    ].astype("float32")

    # ---- categorical token index: EventId ----
    token_idx = None
    ...
    data = {
        ...
        "y": np.array([int(tr["isAbnormal"])], dtype="int64"),  # <-- lowercase
        "TraceId": task_id,
    }
